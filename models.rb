 	...
 	def self.send_due_capsules_to_tagged_users!
  	...
  	# get the image path
  					...
  					due_capsule.taggings.each do |tag|
  						...
  						user = 
  						EmailSender.send(:address => tag.user.first.email, :subject => "Here's your Throwback!", :body => "You've received a Throwback from " + owner +". Click the link below to view your photo.
